...
const Hero = ({ data, grade }) => {
    ...
    async function checkIfPhoneExists(phone) {
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_URL}/api/students/admissions?registerPhone=${phone}`
            );
    ...
    const handleForm = async (e) => {
        ...
        if (regexPhoneNumber.test(inputRegisterPhone)) {
            setIsLoading(true);
            const students = await checkIfPhoneExists(inputRegisterPhone);
    ...
    return (
        <>
            <Container className='flex items-start flex-wrap lg:flex-nowrap lg:gap-10 mt-8 lg:mt-0'>
